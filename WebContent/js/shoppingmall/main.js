$(document).ready(function () {
    ...
    $('.add-cart-large').each(function (i, el) {
        $(el).click(function () {
            ...
            var price = $(this).parent().find('input').get(0).value;
    ...
    $('.add_to_cart').click(function () {
        ...
        var price = $(this).parent().find('input').get(0).value;
        if(!IsNum(price) || price == '' || price == 0){
        	alert('The number you entered is not correct!');
        	return;
        }
        $('body').append('<div class="floating-cart"></div>');
        var cart = $('div.floating-cart');
    ...
    $('#checkout').click(function () {
    	var priceList = $(this).next().next().find('strong');
        var nameList = $(this).next().next().find('span');
        var totalPrice = 0;
        var jsonObj = {goodsArray:[]};
        for(var i = 1; i < nameList.size(); i++){
        	jsonObj.goodsArray.push({
				"goodsName": nameList.get(i).innerHTML ,
				"price": priceList.get(i - 1).innerHTML
        	});
        	totalPrice += Number(priceList.get(i - 1).innerHTML);
        }
        $.ajax( {
			type : 'get',
			url : 'hasEnoughPoints.do?t=' + new Date().getTime(),
			dataType: "text",
			data : { 
			...
			success : function(msg) {				      
				if(msg == 'yes'){
					if(confirm('Are you want to use ' + totalPrice + ' to buy those goods?')){
			        	document.getElementById("jsonObj").value = JSON.stringify(jsonObj);
			        	doFormSubmit('shoppingmallForm','doSaveConsume.do');
			        }
				}
				else{
